...
#define short_options "c:de:f:hl:p:r:u:t:U:P:"
  ...
  if (eXosip_listen_addr (context_eXosip, IPPROTO_TCP, NULL, port, AF_INET, 0)) {
  ...
  for (;;) {
    ...
    case EXOSIP_REGISTRATION_FAILURE:
      regparam.auth = 1;
      syslog_wrapper (LOG_INFO, "registrered failed");
      break;
    case EXOSIP_CALL_INVITE: {
      sdp_message_t *sdp = eXosip_get_remote_sdp(context_eXosip, event->did);
      syslog_wrapper (LOG_INFO, "invite received:\n %s\n\n", event->response->message);
      break;
    }
    case EXOSIP_CALL_REINVITE:
      syslog_wrapper (LOG_INFO, "reinvite received");
      break;
    default:
      syslog_wrapper (LOG_INFO, "recieved unknown eXosip event (type, did, cid) = (%d, %d, %d)", event->type, event->did, event->cid);
